...
public class FxmTestMouse : MonoBehaviour
{
	...
	public void UpdateCamera(bool bOnlyZoom)
	{
		...
		m_fDistance = Mathf.Clamp(m_fDistance - UnityEngine.Input.GetAxis("Mouse ScrollWheel") * m_fWheelSpeed * num, m_fDistanceMin, m_fDistanceMax);
		...
		{
			m_fXRot += UnityEngine.Input.GetAxis("Mouse X") * m_fXSpeed * 0.02f;
			m_fYRot -= UnityEngine.Input.GetAxis("Mouse Y") * m_fYSpeed * 0.02f;
		}
		...
		Quaternion rotation = Quaternion.Euler(m_fYRot, m_fXRot, 0f);
		if (m_bRaycastHit && Physics.Linecast(m_TargetTrans.position, Camera.main.transform.position, out RaycastHit hitInfo))
		{
			m_fDistance -= hitInfo.distance;
		}
		Vector3 point = new Vector3(0f, 0f, 0f - m_fDistance);
		Vector3 position = rotation * point + m_TargetTrans.position;
	...
	private void UpdatePosition(Transform camera)
	{
		if (m_bHandEnable)
		{
			...
			{
				m_OldMousePos = UnityEngine.Input.mousePosition;
				m_bLeftClick = true;
			}
